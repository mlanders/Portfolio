...
function ProjectCard({ project }) {
    return (
        <Fade>
            <div className="flex items-start justify-between bg-[#eef1ef] max-w-[800px] w-full my-[5px] mb-[30px] rounded animate-fadeIn">
                ...
                <div className="box-border flex max-w-[500px] w-full font-normal flex-col justify-between p-6 text-gray-700">
                    <div>
                        <h2 className="text-4xl font-semibold text-gray-900 mb-4">{project.name}</h2>
                        <p className="text-2xl mb-4 leading-relaxed">{project.description}</p>
                        <div className="mt-4">
                            {project.tags.map((tag, index) => (
                                <i
                                    className={`${tag} text-4xl text-gray-700 mr-5`}
                                    key={index}
                                />
                            ))}
                        </div>
